...
בכל שינוי באתר יש להעלות את מספר הגרסה **בשני מקומות** (אותו מספר):

1. `data.js` — השורה `const APP_VERSION = "x.y.z"`
2. `sw.js` — השורה `const VERSION = "x.y.z"`

זה מה שגורם לאפליקציות מותקנות להתעדכן ולהתרענן אוטומטית. הגרסה הנוכחית מוצגת בתחתית האתר — כך אפשר לוודא בטלפון שהעדכון הגיע.
