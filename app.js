...
function buildRecommendations(spot, wave, wind, month, pressureTrend) {
  ...
  if (spot.type === "lake") {
    ...
    recs.push(["🎣 לורים במים מתוקים", "גם באגם יש דיג לורים: שפמנונים תוקפים סיליקונים כהים וצלצלים בשעות החשיכה, וביניות — כפיות זעירות ולורים קטנים בזרם."]);
  ...
  // הבובה המומלצת להיום — נבחרת לפי מצב הים והעונה (מופיעה תמיד)
  let lureTip;
  if (isAutumn) {
    lureTip = "פופר או סטיקבייט על פני המים בזריחה — עונת האינטיאס. אם רואים להקה מרתיחה את המים: כף מתכת כסופה באיסוף מהיר.";
  } else if (isWinter && wave >= 1.2) {
    lureTip = "מינו (בובה שוחה) 9–12 ס\"מ בצבע טבעי — הקלאסיקה ללברק בים גלי. במים עכורים מאוד: וויברציה.";
  } else if (wave < 0.5) {
    lureTip = "מים צלולים ושטוחים: סטיקבייט בצבע טבעי, סיליקונים קטנים על ראש ג'יג וחוט דק — הדגים רואים הכל היום.";
  } else if (wave <= 1.4) {
    lureTip = "כף מתכת כסופה 20–40 גרם או שור-ג'יג להטלות רחוקות, וסיליקון על ראש ג'יג צמוד לקרקעית ליד סלעים.";
  } else {
    lureTip = "מים עכורים מהגלים: וויברציה שמשדרת רעידות, או בובות בצבעים זוהרים/צהובים שנראות מרחוק.";
  }
  recs.push(["🎣 הבובות להיום", lureTip]);

  if (wave <= 1.0 && wind < 25) {
    recs.push(["🚤 יום טוב לז'רזור", "ים נוח ליציאה בסירה או קיאק — גרירת בובות שוחות צוללות וכפיות ב-2–7 קשר מול קו החוף. ציפורים צוללות = להקת טורפים."]);
  }

  if (isSummer && wave < 1.5) {
    recs.push(["🌅 טורפי קיץ על לורים", "ספינינג בזריחה: ברקודות ולוקוסים תוקפים סיליקונים ובובות שוחות ליד סלעים, שוברי גלים וכניסות המרינות."]);
  }
